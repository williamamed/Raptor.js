...
module.exports={
	...
	devMode:function(){
		...
		var conf={
	        script: baseDir+'/'+path.relative(baseDir,main),
	        ignoreRoot:['.git','.nyc_output','.sass-cache','bower_components','coverage'],
	        ignore: ["*Node/Resources/*"],
	        verbose: true,
	        //exec: process.execPath,
	        
	        cwd: baseDir,
	        //exec: 'node '+main+' '+baseDir+'',
	        //exec: 'node "'+path.relative(baseDir,main)+'" ',
	        watch: ['src/','config/','node_modules/raptorjs/lib/*'],
	      }
	    //  console.log('Current directory: ' + path.dirname(process.execPath),path.relative(baseDir,main));
		nodemon(conf).on('start', function () {
		   console.log(format.get('Nodemon started',format.YELLOW));
		   console.log(format.get(msg,format.GREEN));
		}).on('crash', function () {
		   console.log('script crashed for some reason');
		}).on('restart',function(){
		   console.log(format.get('Raptor.js app restarting',format.YELLOW));
