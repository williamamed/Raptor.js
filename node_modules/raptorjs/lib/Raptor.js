...
module.exports={
	...
	*/
	main:function(basepath){
		...
		global.$set=function(name,obj){
		    self.container[name]=obj
		    self.emit('ioc:'+name+'.ready',self.container[name])
		}
			  		...
			  		.filter(function(fileNode) {
						...
						return (fileNode.indexOf('.') !== 0) && (fileNode !== 'index.js') && (sub=='Node')
				  		...
				  		.forEach(function(comp) {
				  		    ...
				  		    if(!fs.existsSync(path.join(rutaSrc,file,comp,'manifest.json'))){
				  		        var data={
				  		            state: true,
				  		            name: comp,
				  		            version: '1.0.0'
				  		        }
				  		        fs.writeFileSync(path.join(rutaSrc,file,comp,'manifest.json'),JSON.stringify(data,null, 2))
				  		    }
				  				...
				  				if(typeof index == 'function'){
				  					if(!me.bundles[comp]){
					  					me.bundles[comp]={
							   	 			...
							   	 			vendor: file,
							   	 			manifest: require(path.join(rutaSrc,file,comp,'manifest.json')),
							   	 			main: index,
							   	 			controllers:[],
											models:{},
											init:false,
										...
										if(main.configure && me.on){
											...
											me.emit('before:module.config',comp)
											main.configure(me)
											me.bundles[comp].hooks.config('after').resolve(me)
											me.emit('after:module.config',comp)
										}*/
				  					}else{
			...
			for (var bundle in me.bundles) {
			    if(!me.bundles[bundle].init){
    			    ...
    			    for (var depend in me.bundles[bundle].manifest.require?me.bundles[bundle].manifest.require:{}) {
    			        ...
    			        var operat=requestDepend.split('@')[0]
    			        var versionDepend=requestDepend.split('@').length>1?requestDepend.split('@')[1]:requestDepend
    			        switch(operat){
    			            case '==':{
    			                if(!(me.bundles[depend] && me.bundles[depend].manifest.state && me.versionCompare(me.bundles[depend].manifest.version,versionDepend) == 0)){
    			                    fullfill=false
    			                }
    			                break;
    			            }
        			...
        			if(main.configure && me.on){
    					...
    					me.emit('before:'+bundle+'.config',me.bundles[bundle])
    					main.configure(me)
    					me.bundles[bundle].hooks.config('after').resolve(me)
    					me.emit('after:'+bundle+'.config',me.bundles[bundle])
    				}
    				me.bundles[bundle].init=true;
			    }
    		}
			
			me.emit('after:config')
				  ...
				  .forEach(function(file) {
				  	...
				  	if(typeof controller == 'function'){
				  		...
				  		if(claseES6.configure){
				  			...
				  			var routes=R.getAnnotationRouteConfig(path.join(pathController,file),claseES6)
				  			
				  			if(typeof routes=="object"){
				  			    claseES6.routes(routes)
	...
	*/
	mainNodeBundle:function(mainClass,R,bundle){
		return function(request,response,next){
			...
			if(R.options.mode=='development' && !response.req.xhr && response.req.method=='GET' && (!response.get('Content-type') || response.get('Content-type').indexOf('text/html')!=-1)){
	...
	*/
			...
			if(me.bundles[name] && fs.existsSync(path.join(me.basePath,'src',me.bundles[name].path)) && fs.existsSync(path.join(me.basePath,'src',me.bundles[name].path)+"/Migration/skeleton.js")){
				...
				var routing=function(tableNames) {
    				    ...
    				    if(me.database.sequelize.options.dialect=='postgres' || me.database.sequelize.options.dialect=='oracle')
        					for(var i=0;i<migration.watch.length;i++){
        				        (function(){
        						    ...
        						    .then(function(max){
        						        return me.database.sequelize.query("ALTER SEQUENCE IF EXISTS "+table+"_id_seq START WITH "+(max[0][0].max+1)+" RESTART");
        						    })
        						    .then(function(){
					...
					if(action=='create'){
						...
						if(flagExist==0){
							migration[action](queryInterface,me,data,doneCreate)
	...
	getAnnotationRouteConfig:function(file,scope){
        ...
        var reader=this.annotationFramework.reader;
        reader.parse(file)
        ...
        methodAnnotations.forEach((annotation) => {
            ...
            if (annotation.annotation === 'Route'){
                ...
                if(annotation.method)
                    routes[prefix+annotation.value]={ method:annotation.method, action: scope?scope[annotation.target]:annotation.target }
                else
                    routes[prefix+annotation.value]={ method:'all', action: scope?scope[annotation.target]:annotation.target }
            }
        })
	...
	_configReadAnnotation:function(){
        ...
        this.annotationFramework.registry.registerAnnotation(require.resolve('raptorjs/lib/Annotation/Route'))
