...
module.exports = {
	...
	create: function (queryInterface, R, data, done) {
		...
		return this.createTableFromModel(R.getModels('TroodonNode').security_category)
			.then(result => {
			...
			.then(result => {
				...
				//Modificado en la version 2.0.0 para eliminar el error al insertar un datetime
				return $injector('Troodon_security_user').bulkCreate(data['security_user'], {
					hooks: false
				})
			})
			.then(result => {
	...
	destroy: function (queryInterface, R, data, done) {
		return queryInterface.dropTable('security_trace')
			.then(p => {
	...
	export: function (queryInterface, R, done, save) {
		...
		return R.getModels('TroodonNode').security_category.findAll()
			.then(result => {
